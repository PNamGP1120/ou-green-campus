...
const DEFAULT_IMAGE =
  "https://cdn-icons-png.flaticon.com/512/337/337946.png"; // ảnh mặc định cho tài liệu
...
const Documents = () => {
  ...
  const loadDocs = async () => {
    try {
      ...
      if (page === 1) setDocs(data);
      else setDocs((prev) => [...prev, ...data]);

      if (data.length === 0) setPage(0);
    } catch (ex) {
      console.error("Lỗi load documents:", ex);
      setDocs([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="container py-4">
      <h2 className="text-primary fw-bold text-center mb-4">
        <i className="bi bi-journal-text me-2"></i> Thư viện Tài liệu
      </h2>
      ...
      <Row>
        {docs.map((d) => (
          <Col className="p-2" md={4} xs={12} key={d.id}>
            <Card className="h-100 shadow-sm">
              ...
              <Card.Body>
                ...
                >
                  <i className="bi bi-file-earmark-arrow-down me-1"></i> Xem / Tải về
                </a>
              </Card.Body>
